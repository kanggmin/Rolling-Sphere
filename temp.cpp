...
point3 sphere8_points[24];
color3 sphere8_colors[24];
point3 sphere128_points[384];
color3 sphere128_colors[384];
...
void floor()
{
    floor_colors[0] = vertex_colors[3]; floor_points[0] = point3(5.0, 0.0, 8.0);
    floor_colors[1] = vertex_colors[3]; floor_points[1] = point3(5.0, 0.0, -4.0);
    floor_colors[2] = vertex_colors[3]; floor_points[2] = point3(-5.0, 0.0, -4.0);

    floor_colors[3] = vertex_colors[3]; floor_points[3] = point3(-5.0, 0.0, -4.0);
    floor_colors[4] = vertex_colors[3]; floor_points[4] = point3(-5.0, 0.0, 8.0);
    floor_colors[5] = vertex_colors[3]; floor_points[5] = point3(5.0, 0.0, 8.0);
}
//----------------------------------------------------------------------------
void fileIn(string filepath, point3 points[], color3 colors[])
{
    ...
    //NUMBER OF CIRCLES FROM FILE
    int num;
    file >> num;


    //CREATE A CIRCLE AND INPUT THE COORDINATES AND RADIUS THEN ADD TO VECTOR
    for (int i = 0; i < sizeof(points); ++i) {
        GLfloat n, x, y, z;
        file >> n >> x >> y >> z;
        points[i] = point3(x, y, z);
        colors[i] = vertex_colors[2];
    }
...
void init()
{
    ...
    fileIn("sphere8.txt", sphere8_points, sphere8_colors);
    fileIn("sphere128.txt", sphere128_points, sphere128_colors);
    ...
    glBufferData(GL_ARRAY_BUFFER,
        sizeof(point3) * sizeof(sphere8_points) + sizeof(color3) * sizeof(sphere8_points),
        NULL, GL_STATIC_DRAW);
    glBufferSubData(GL_ARRAY_BUFFER, 0,
        sizeof(point3) * sizeof(sphere8_points), sphere8_points);
    glBufferSubData(GL_ARRAY_BUFFER,
        sizeof(point3) * sizeof(sphere8_points),
        sizeof(color3) * sizeof(sphere8_colors),
        sphere8_colors);
...
void display(void)
{
  ...
  // The set-up below gives a new scene (scene 2), using Correct LookAt().
    mv = LookAt(eye, at, up) * Translate(-7.0f, -3.0f, 10.0f) * Scale(1.4f, 1.4f, 1.4f);
    // * RotateY(angle);
//
...
//
    ...
    */
    ...
    if (cubeFlag == 1) // Filled cube
        ...
        glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
    drawObj(sphere_buffer, sizeof(sphere8_points));  // draw the cube

    /*----- Set up the Mode-View matrix for the floor -----*/
     // The set-up below gives a new scene (scene 2), using Correct LookAt() function
    mv = LookAt(eye, at, up) * Translate(7.0f, 3.0f, -10.0f) * Scale(1.6f, 1.5f, 3.3f);
    //
    // The set-up below gives the original scene (scene 1), using Correct LookAt()
